...
/*
 * Names: Jeremiah Kwon and Sudeep Annem
 * Partners: jskwon2, sannem2
 * Date: 4/12/19
 *
 * The following program provides the code for functions that can do some of the
 * following such as creating a horizontal module, determining if a node is a leaf,
 * an internal node, or if the node is in the subtree of another node. Other
 * functions include being able to rotate a module, changing how it is cut, moving a
 * whole submodule and individual modules around. It can also get the postfix
 * expression and find the total number of resources each module takes up.
 *
 */
...
int is_internal_node(node_t* ptr) {
	// Checks if the node is an internal node, which means it is not a leaf nor root
	if(!is_leaf_node(ptr))
  		return 1;

	// Returns a 0 if the node is not an internal node
	return 0;
}
...
int is_in_subtree(node_t* a, node_t* b) {
	...
	// Checks to see if the parent of the nodes above b is equal to node a
	...
	// Returns 0 if the given subtree at b is not in the subtree of a
	return 0;
}
...
void rotate(node_t* ptr) {
	...
	// Rotates the node by switching the width and height of a module
	temp = ptr->module->w;
...
void recut(node_t* ptr) {
	// Prewritten code that says to ignore all leaf nodes
	if(!is_internal_node(ptr)) return;
	assert(ptr->module == NULL && ptr->cutline != UNDEFINED_CUTLINE);

	// Changes the cutlines of a module
	if(ptr->cutline == V)
		ptr->cutline = H;
	else
		ptr->cutline = V;
...
void swap_module(node_t* a, node_t* b) {
	// Checks to see if node a and b are leaf nodes
	...
	// Initializes the needed variables and stuff
	module_t* temp;

	// Swaps the modules in the code below
	temp = a->module;
	a->module = b->module;
	b->module = temp;
}
...
void swap_topology(node_t* a, node_t* b) {
	// Checks to see if a and b are root nodes and if they are nodes themselves.
	// After, it checks if they are in each other's subtree
	if(a == NULL || b == NULL) return;
	if(a->parent == NULL || b->parent == NULL) return;
	if(is_in_subtree(a, b) || is_in_subtree(b, a)) return;
	assert(a->parent != NULL && b->parent != NULL);

	// Creates the variables needed to swap topologies
	node_t* temp;
	int a_direction = -1, b_direction = -1;	// 0 is left and 1 is right

	// Determines whether node a is to the left or right of the parent node
	if(a->parent->left == a)
		a_direction = 0;
	else
		a_direction = 1;
	...
	else if(a_direction == 0 && b_direction == 1) {
		a->parent->left = b;
		b->parent->right = a;
	}
	else if(a_direction == 1 && b_direction == 0) {
		a->parent->right = b;
		b->parent->left = a;
	}
	else {
...
void postfix_traversal(node_t* ptr, int* nth, expression_unit_t* expression) {
	// Checks if ptr is NULL. If it is, then the function returns
	if(ptr == NULL) return;
...
int get_total_resource(node_t* ptr)
{
	...
	// Base case
	if(is_leaf_node(ptr)) {
		count = (ptr->module)->resource;
		return count;
	}

	// Traverses the BST with the post order traversal and sums the resources
	if(ptr->left != NULL)
		count += get_total_resource(ptr->left);
	if(ptr->right != NULL)
		count += get_total_resource(ptr->right);

	// Returns the sum of the resources
	return count;
}
...
node_t* init_slicing_tree(node_t* par, int n) {
	// Checks if n is greater or equal to 0
	...
	if(n == num_modules - 1) {
		node->left = NULL;
		node->right = NULL;
		node->cutline = UNDEFINED_CUTLINE;
		node->module = (module_t *)malloc(sizeof(module_t));
		node->module->idx = n;
		return node;
	}
	...
	node->right->cutline = UNDEFINED_CUTLINE;
	node->right->module = (module_t *)malloc(sizeof(module_t));
	node->right->module->idx = n;

	// Left child
	node->left = init_slicing_tree(node, n + 1);
